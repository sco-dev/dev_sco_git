sap.ui.define([
    ...
    return {
        ...
        httpCall: function (params) {
            ...
            //misma vista de cuando se invoca
            let view = that.view;
            view.setBusy(true);
            ...
            if (!window.location.href.includes('https')) {
                ...
                // url = "https://salfacloudqas.salfa.cl/sap/fiori/app00230crearpedidoventa" + url;
            }
            ...
            return $.ajax({
                ...
                that._pendingRequests--;
                //esperamos porque si se vuelve a setear busy muy pronto no funciona
                setTimeout(function(){
        ...
        copyObject: function (originKeys, destinationKeys, originData) {
            if(originKeys.length !== destinationKeys.length) {
                return {};
            } else {
                let destObj = {};
                for (var i2 = 0; i2 < originKeys.length; i2++) {
                    destObj[destinationKeys[i2]] = originData[originKeys[i2]];
                }
                return destObj;
            }
        },

        copyObjectArray: function (originKeys, destinationKeys, originArray, addEmpty) {
            if(originKeys.length !== destinationKeys.length) {
                return [];
            }else {
                ...
                for (var i = 0; i < originArray.length; i++) {
                    let destObj = {};
                    for (var i2 = 0; i2 < originKeys.length; i2++) {
                        destObj[destinationKeys[i2]] = originArray[i][originKeys[i2]];
                    }
                    destinationArray.push(destObj);
                }
                if(addEmpty){
                    destinationArray.unshift({[destinationKeys[0]]:'',[destinationKeys[1]]:'Seleccione'});
                }
